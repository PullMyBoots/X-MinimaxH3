...
def download_from_modelscope(model_id, origin_file_path, local_dir):
    # H3 Video Service always supplies pinned local weights. Keep the optional
    # upstream downloader lazy so a local inference worker does not require the
    # large ModelScope client stack merely to import the model loader.
    try:
        from modelscope import snapshot_download
    except ImportError as error:
        raise RuntimeError(
            "ModelScope is only required when downloading weights at runtime"
        ) from error
